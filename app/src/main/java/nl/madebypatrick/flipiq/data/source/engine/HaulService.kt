...
interface HaulService {
    suspend fun scan(jpeg: ByteArray, rotationDegrees: Int): List<HaulItem>
}

/** No-op when the engine isn't configured. */
class NoopHaulService : HaulService {
    override suspend fun scan(jpeg: ByteArray, rotationDegrees: Int): List<HaulItem> = emptyList()
}
...
class EngineHaulService(
    ...
    override suspend fun scan(jpeg: ByteArray, rotationDegrees: Int): List<HaulItem> =
